...
class Blob:
    ...
    def add(self, x, y):
        self._x = (self._x * self._pixels + x * 1) / (self._pixels + 1)
        self._y = (self._y * self._pixels + y * 1) / (self._pixels + 1)
        self._pixels += 1

    # Returns the mass of this blob, ie, the number of pixels in it.
    def mass(self):
        return self._pixels  # the mass is the number of the pixels
